...
func recoverObjectAction(ctx *cli.Context) error {
	...
	for range taskCheckTicker.C {
		...
		if stats.GetRecoveryProcessCount() > 0 {
			log.Infof("Processing recovery tasks, update every 5 seconds. Waiting Tasks Number: %d. Recovered: %s. Failed: %s", stats.GetRecoveryProcessCount(), stats.GetRecoverySucceedList(), stats.GetRecoveryFailedList())
		} else {
			log.Infof("Finished process all recovery objects task on background. Recovered: %s. Failed: %s \n", stats.GetRecoverySucceedList(), stats.GetRecoveryFailedList())
			break
		}
	}
