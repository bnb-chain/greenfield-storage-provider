...
var (
	ErrReceiveTaskDangling    = gfsperrors.Register(BaseCodeSpace, http.StatusInternalServerError, 99081, "OoooH... request lost")
	ErrReceiveExhaustResource = gfsperrors.Register(BaseCodeSpace, http.StatusServiceUnavailable, 99082, "server overload, try again later")
)
...
func (g *GfSpBaseApp) GfSpReplicatePiece(
	...
	*gfspserver.GfSpReplicatePieceResponse, error) {
	...
	log.CtxInfow(ctx, "succeed to replicate piece")
...
func (g *GfSpBaseApp) GfSpDoneReplicatePiece(
	...
	*gfspserver.GfSpDoneReplicatePieceResponse, error) {
	...
	log.CtxInfow(ctx, "succeed to complete replicate pieces")
