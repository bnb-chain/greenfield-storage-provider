...
var (
	ErrDownloadTaskDangling    = gfsperrors.Register(BaseCodeSpace, http.StatusInternalServerError, 99301, "OoooH... request lost")
	ErrDownloadExhaustResource = gfsperrors.Register(BaseCodeSpace, http.StatusServiceUnavailable, 99302, "server overload, try again later")
)
...
func (g *GfSpBaseApp) GfSpGetChallengeInfo(
	...
	*gfspserver.GfSpGetChallengeInfoResponse, error) {
	task := req.ChallengePieceTask
	if task == nil {
		log.Error("failed to challenge piece, task pointer dangling")
...
func (g *GfSpBaseApp) OnChallengePieceTask(
	...
	if task == nil || task.GetObjectInfo() == nil {
		log.Error("failed to challenge piece, challenge piece task pointer dangling")
