...
func (g *GfSpBaseApp) OnAskTask(
	...
	log.CtxDebugw(ctx, "dispatch task info", "task_retry", gfspTask.GetRetry(),
		...
		"task_create_time", gfspTask.GetCreateTime(), "task_update_time", gfspTask.GetUpdateTime(),
		"task_timeout", gfspTask.GetTimeout())
	return gfspTask, nil
}
...
func (g *GfSpBaseApp) GfSpReportTask(
	...
	*gfspserver.GfSpReportTaskResponse, error) {
	...
	case *gfspserver.GfSpReportTaskRequest_UploadObjectTask:
		...
		log.CtxInfow(ctx, "begin to handle reported task", "task_error", task.Error())
