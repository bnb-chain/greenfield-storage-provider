...
var (
	ErrUploadTaskDangling  = gfsperrors.Register(BaseCodeSpace, http.StatusInternalServerError, 99601, "OoooH... request lost")
	ErrUnsupportedTaskType = gfsperrors.Register(BaseCodeSpace, http.StatusNotFound, 99602, "unsupported task type")
	ErrNoTaskMatchLimit    = gfsperrors.Register(BaseCodeSpace, http.StatusNotFound, 99603, "no task to dispatch below the require limits")
	ErrNoSuchTask          = gfsperrors.Register(BaseCodeSpace, http.StatusNotFound, 99604, "no such task")
)
...
func (g *GfSpBaseApp) OnBeginUploadObjectTask(
	...
	if err != nil {
		log.CtxErrorw(ctx, "failed to check upload object task", "error", err)
		return err
	}
	log.CtxDebugw(ctx, "succeed to check upload object task")
	return nil
}
