...
func (g *GfSpBaseApp) startServices(ctx context.Context) {
	for i, service := range g.services {
		if err := service.Start(ctx); err != nil {
			log.Errorf("modular %s starts error: %v", service.Name(), err)
			g.services = g.services[:i]
			g.appCancel()
			break
		} else {
			log.Infof("modular %s starts successfully", service.Name())
		}
	}
}
...
func (g *GfSpBaseApp) StopServices(ctx context.Context) {
	...
	if errors.Is(gCtx.Err(), context.Canceled) {
		log.Infow("modular stop working", "stop service timeout", DefaultStopTime)
	} else if errors.Is(gCtx.Err(), context.DeadlineExceeded) {
		log.Error("modular while stopping service, killing instance manually")
	}
}
...
func (g *GfSpBaseApp) stopServices(ctx context.Context, cancel context.CancelFunc) {
	for _, service := range g.services {
		if err := service.Stop(ctx); err != nil {
			log.Errorf("modular %s stops failure: %v", service.Name(), err)
		} else {
			log.Infof("modular %s stops successfully!", service.Name())
		}
	}
	cancel()
}
