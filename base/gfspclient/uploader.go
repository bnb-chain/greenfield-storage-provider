...
func (s *GfSpClient) UploadObject(ctx context.Context, task coretask.UploadObjectTask, stream io.Reader) error {
	...
	metrics.PerfPutObjectTime.WithLabelValues("client_put_object_prepare_end").Observe(float64(time.Now().UnixMilli()-task.GetCreateTime()) / 1000)
	for {
		...
		metrics.PerfPutObjectTime.WithLabelValues("client_put_object_read_data_end").Observe(float64(time.Now().UnixMilli()-task.GetCreateTime()) / 1000)
		sendSize += n
		if streamErr == io.EOF {
			if n != 0 {
				...
				metrics.PerfPutObjectTime.WithLabelValues("client_put_object_send_end").Observe(float64(time.Now().UnixMilli()-task.GetCreateTime()) / 1000)
				if err != nil {
			...
			metrics.PerfPutObjectTime.WithLabelValues("client_put_object_send_last_end").Observe(float64(time.Now().UnixMilli()-task.GetCreateTime()) / 1000)
			if closeErr != nil {
