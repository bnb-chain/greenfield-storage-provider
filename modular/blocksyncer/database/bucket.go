...
func (db *DB) UpdateStorageSizeToSQL(ctx context.Context, objectID common.Hash, bucketName, operation string) (string, []interface{}) {
	...
	sql := `UPDATE buckets SET storage_size = storage_size %s IFNULL(CONVERT((SELECT payload_size FROM %s WHERE object_id = ?), DECIMAL(65,0)), 0) WHERE bucket_name = ?`
...
func (db *DB) UpdateChargeSizeToSQL(ctx context.Context, objectID common.Hash, bucketName, operation string) (string, []interface{}) {
	tableName := bsdb.GetObjectsTableName(bucketName)
	sql := `UPDATE buckets SET charge_size = charge_size %s CASE WHEN (CAST(IFNULL((SELECT payload_size FROM %s WHERE object_id = ?), 0) AS DECIMAL(65,0)) < 128000) THEN CAST(128000 AS DECIMAL(65,0)) ELSE CAST(IFNULL((SELECT payload_size FROM %s WHERE object_id = ?), 0) AS DECIMAL(65,0)) END WHERE bucket_name = ?`
