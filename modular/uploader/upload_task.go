...
var (
	ErrDanglingDownloadTask = gfsperrors.Register(UploadModularName, http.StatusNotFound, 110001, "OoooH... request lost, try again later")
	ErrNotCreatedState      = gfsperrors.Register(UploadModularName, http.StatusBadRequest, 110002, "object not created state")
	ErrRepeatedTask         = gfsperrors.Register(UploadModularName, http.StatusBadRequest, 110003, "put object request repeated")
	ErrExceedTask           = gfsperrors.Register(UploadModularName, http.StatusInsufficientStorage, 110004, "OoooH... request exceed, try again later")
	ErrInvalidIntegrity     = gfsperrors.Register(UploadModularName, http.StatusBadRequest, 110005, "invalid payload data integrity hash")
	ErrClosedStream         = gfsperrors.Register(UploadModularName, http.StatusInternalServerError, 110006, "upload payload data stream exception")
	ErrPieceStore           = gfsperrors.Register(UploadModularName, http.StatusInternalServerError, 115101, "server slipped away, try again later")
	ErrSinger               = gfsperrors.Register(UploadModularName, http.StatusInternalServerError, 111001, "server slipped away, try again later")
	ErrGfSpDB               = gfsperrors.Register(UploadModularName, http.StatusInternalServerError, 115001, "server slipped away, try again later")
)
