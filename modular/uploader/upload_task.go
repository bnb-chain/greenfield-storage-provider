...
func (u *UploadModular) PreUploadObject(ctx context.Context, uploadObjectTask coretask.UploadObjectTask) error {
	...
	metrics.PerfPutObjectTime.WithLabelValues("uploader_put_object_report_upload_manager_end").Observe(time.Since(time.UnixMilli(uploadObjectTask.GetCreateTime())).Seconds())
	if err != nil {
...
func (u *UploadModular) HandleUploadObjectTask(ctx context.Context, uploadObjectTask coretask.UploadObjectTask, stream io.Reader) error {
	...
	metrics.PerfPutObjectTime.WithLabelValues("uploader_put_object_begin_from_task_create").Observe(time.Since(time.UnixMilli(uploadObjectTask.GetCreateTime())).Seconds())
	defer func() {
		if err != nil {
		...
		metrics.PerfPutObjectTime.WithLabelValues("uploader_put_object_end_from_task_create").Observe(time.Since(time.UnixMilli(uploadObjectTask.GetCreateTime())).Seconds())
		go func() {
			metrics.PerfPutObjectTime.WithLabelValues("uploader_put_object_before_report_manager_end").Observe(time.Since(time.UnixMilli(uploadObjectTask.GetCreateTime())).Seconds())
			_ = u.baseApp.GfSpClient().ReportTask(context.Background(), uploadObjectTask)
			metrics.PerfPutObjectTime.WithLabelValues("uploader_put_object_after_report_manager_end").Observe(time.Since(time.UnixMilli(uploadObjectTask.GetCreateTime())).Seconds())
		}()
	}()
	startTime := time.Now()
	for {
		...
		metrics.PerfPutObjectTime.WithLabelValues("uploader_put_object_server_read_data_end").Observe(time.Since(time.UnixMilli(uploadObjectTask.GetCreateTime())).Seconds())
		readSize += readN
		data = data[0:readN]
		...
		if err == io.EOF {
			...
			metrics.PerfPutObjectTime.WithLabelValues("uploader_put_object_sign_integrity_end").Observe(time.Since(time.UnixMilli(uploadObjectTask.GetCreateTime())).Seconds())
			if err != nil {
			...
			metrics.PerfPutObjectTime.WithLabelValues("uploader_put_object_set_integrity_end").Observe(time.Since(time.UnixMilli(uploadObjectTask.GetCreateTime())).Seconds())
			if err != nil {
		...
		err = u.baseApp.PieceStore().PutPiece(ctx, pieceKey, data)
		metrics.PerfPutObjectTime.WithLabelValues("uploader_put_object_server_put_piece_cost").Observe(time.Since(startPutPiece).Seconds())
		metrics.PerfPutObjectTime.WithLabelValues("uploader_put_object_server_put_piece_end").Observe(time.Since(time.UnixMilli(uploadObjectTask.GetCreateTime())).Seconds())
		if err != nil {
