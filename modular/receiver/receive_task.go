...
var (
	ErrDanglingTask        = gfsperrors.Register(ReceiveModularName, http.StatusNotFound, 80001, "OoooH... request lost, try again later")
	ErrRepeatedTask        = gfsperrors.Register(ReceiveModularName, http.StatusBadRequest, 80002, "request repeated")
	ErrExceedTask          = gfsperrors.Register(ReceiveModularName, http.StatusInsufficientStorage, 80003, "OoooH... request exceed, try again later")
	ErrUnfinishedTask      = gfsperrors.Register(ReceiveModularName, http.StatusForbidden, 80004, "replicate piece unfinished")
	ErrInvalidDataChecksum = gfsperrors.Register(ReceiveModularName, http.StatusBadRequest, 80005, "verify data checksum failed")
	ErrSinger              = gfsperrors.Register(ReceiveModularName, http.StatusInternalServerError, 80101, "server slipped away, try again later")
	ErrPieceStore          = gfsperrors.Register(ReceiveModularName, http.StatusInternalServerError, 85101, "server slipped away, try again later")
	ErrGfSpDB              = gfsperrors.Register(ReceiveModularName, http.StatusInternalServerError, 85201, "server slipped away, try again later")
)
