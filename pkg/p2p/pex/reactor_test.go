...
import (
	...
	"github.com/bnb-chain/greenfield-storage-provider/pkg/p2p/libs/crypto/ed25519"
	"github.com/bnb-chain/greenfield-storage-provider/pkg/p2p/p2ptest"
...
func setupSingle(ctx context.Context, t *testing.T) *singleTestReactor {
	...
	reactor := NewReactor(log.NewNopLogger(), peerManager, chCreator, func(_ context.Context) *p2p.PeerUpdates { return peerUpdates })
...
func setupNetwork(ctx context.Context, t *testing.T, opts testOptions) *reactorTestSuite {
	...
	// reactor
	rts.pexChannels = rts.network.MakeChannelsNoCleanup(ctx, t, ChannelDescriptor())
...
func (r *reactorTestSuite) addNodes(ctx context.Context, t *testing.T, nodes int) {
	...
	for i := 0; i < nodes; i++ {
		...
		nodeID := node.NodeID
		r.pexChannels[nodeID] = node.MakeChannelNoCleanup(ctx, t, ChannelDescriptor())
