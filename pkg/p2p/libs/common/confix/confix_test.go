...
func TestGuessConfigVersion(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			got := GuessConfigVersion(mustParseConfig(t, test.path))
			if got != test.want {
...
func TestApplyFixes(t *testing.T) {
	...
	t.Run("Unknown", func(t *testing.T) {
		err := ApplyFixes(ctx, mustParseConfig(t, "testdata/v31-config.toml"))
	...
	t.Run("TooOld", func(t *testing.T) {
		err := ApplyFixes(ctx, mustParseConfig(t, "testdata/v33-config.toml"))
	...
	t.Run("OK", func(t *testing.T) {
		...
		t.Run("Fixpoint", func(t *testing.T) {
			...
			if err := ApplyFixes(ctx, doc2); err != nil {
				t.Fatalf("ApplyFixes: unexpected error: %v", err)
			}
